...
export const BankStatementScreen: React.FC<BankStatementScreenProps> = ({ onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden select-none">
      ...
      <div 
        className="safe-header px-5 pb-3 bg-[#E5F3E8]/95 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between shrink-0"
      ...
      >
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={onBack}
            className="w-10 h-10 rounded-full bg-white border border-white/80 shadow-sm text-slate-800 flex items-center justify-center transition-all active:scale-90"
          >
            <ArrowLeft size={18} />
          </button>
          <div className="flex flex-col">
            ...
            <p className="text-[11px] text-slate-500 font-bold">Импорт банковских операций</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 sm:p-5 flex flex-col gap-4">
        ...
        {errorMsg && (
          <div className="p-3.5 rounded-2xl bg-rose-50/90 border border-rose-100 text-rose-700 text-xs font-bold flex items-center gap-2.5">
        ...
        {step === 'upload' && (
          <div className="flex flex-col gap-4 max-w-lg mx-auto w-full">
            ...
            {/* Drag & Drop Action Box - Liquid Glass Card */}
            <div 
              onClick={() => fileInputRef.current?.click()}
              onDrop={handleDrop}
              onDragOver={handleDragOver}
              className="card p-8 sm:p-10 flex flex-col items-center text-center gap-4 cursor-pointer transition-all active:scale-[0.99] border-2 border-dashed border-slate-300/80 hover:border-slate-950"
            >
              <div className="w-16 h-16 rounded-2xl bg-slate-950 text-white flex items-center justify-center shadow-md transition-transform active:scale-95">
                {loading ? (
                  <RotateCw size={28} className="animate-spin text-white" />
                ) : (
                  <UploadCloud size={32} />
                )}
              </div>
              ...
              <button
                ...
                className="py-3 px-6 rounded-full bg-slate-950 hover:bg-slate-900 text-white text-xs font-black shadow-md active:scale-95 transition-all mt-1"
              >
                Выбрать файл выписки
              </button>

              <div className="flex flex-wrap items-center justify-center gap-1.5 mt-2">
                <span className="px-3 py-1 rounded-full bg-white/80 backdrop-blur-md text-emerald-800 text-[10px] font-bold border border-white/90 shadow-sm flex items-center gap-1">
                  <CheckCircle2 size={12} /> Авто-категоризация
                </span>
                <span className="px-3 py-1 rounded-full bg-white/80 backdrop-blur-md text-blue-800 text-[10px] font-bold border border-white/90 shadow-sm">
                  Анти-дубликаты
                </span>
                <span className="px-3 py-1 rounded-full bg-white/80 backdrop-blur-md text-purple-800 text-[10px] font-bold border border-white/90 shadow-sm">
                  Чистые названия
                </span>
              </div>
            </div>

            {/* Instructions Guide - Liquid Glass */}
            <div className="card p-5 flex flex-col gap-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-medium text-slate-600">
                <div className="p-3 rounded-2xl bg-white/60 border border-white/80 flex gap-2.5 items-center">
                  <span className="w-6 h-6 rounded-full bg-slate-950 font-black text-white flex items-center justify-center shrink-0 text-xs">1</span>
                  <span>Откройте карту или счет в банке</span>
                </div>
                <div className="p-3 rounded-2xl bg-white/60 border border-white/80 flex gap-2.5 items-center">
                  <span className="w-6 h-6 rounded-full bg-slate-950 font-black text-white flex items-center justify-center shrink-0 text-xs">2</span>
                  <span>Нажмите кнопку <b>«Выписка»</b></span>
                </div>
                <div className="p-3 rounded-2xl bg-white/60 border border-white/80 flex gap-2.5 items-center">
                  <span className="w-6 h-6 rounded-full bg-slate-950 font-black text-white flex items-center justify-center shrink-0 text-xs">3</span>
                  <span>Выберите желаемый период</span>
                </div>
                <div className="p-3 rounded-2xl bg-white/60 border border-white/80 flex gap-2.5 items-center">
                  <span className="w-6 h-6 rounded-full bg-slate-950 font-black text-white flex items-center justify-center shrink-0 text-xs">4</span>
        ...
        ══════════════════════════════════════════════════ */}
        {step === 'preview' && (
          <div className="flex flex-col gap-3.5 max-w-lg mx-auto w-full pb-36">
            
            {/* ── Summary Card (Metadata) - Liquid Dark Glass ── */}
            <div className="card-dark-glass p-5 rounded-[28px] text-white flex flex-col gap-3.5 relative overflow-hidden shrink-0">
              ...
              <div className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-1.5 text-slate-300 font-medium truncate max-w-[240px]">
                  ...
                  <span className="truncate font-bold">{metadata?.period || fileName || 'Период выписки'}</span>
                </div>
                {metadata?.card && (
                  <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/10 text-[10px] text-slate-200 font-bold border border-white/10">
                    <CreditCard size={12} />
              ...
              <div className="grid grid-cols-3 gap-2 pt-2 border-t border-white/10">
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Расход</span>
                ...
                <div className="flex flex-col">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Приход</span>
                ...
                <div className="flex flex-col text-right">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Операций</span>
            ...
            <div className="flex flex-col gap-2 shrink-0">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-8 py-2.5 rounded-2xl bg-white/80 backdrop-blur-xl border border-white/90 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-950/10 transition-all font-bold shadow-sm"
                />
                {searchQuery && (
                  <button
                    type="button"
            ...
            {/* ── Transaction List - Liquid Glass Cards ── */}
            <div className="flex flex-col gap-2.5">
              {filteredList.length === 0 ? (
                <div className="card py-16 flex flex-col items-center justify-center text-center gap-2 text-slate-400">
                ...
                filteredList.map((tx) => {
                  ...
                  return (
                    <div
                      ...
                      className={`card p-3.5 flex items-center justify-between gap-3 cursor-pointer select-none active:scale-[0.99] transition-transform ${
                        tx.isDuplicate 
                          ? 'opacity-65' 
                          : isSelected 
                    ...
                    >
                      ...
                      {/* Neo-Fintech Solid Black Category Avatar */}
                      <div className="w-11 h-11 rounded-full bg-slate-950 text-white flex items-center justify-center shrink-0 shadow-sm">
                        <DynamicIcon name={cat?.icon || 'HelpCircle'} className="w-5 h-5 text-white" />
                      </div>

                      {/* Details */}
                      <div className="flex flex-col flex-1 min-w-0" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs font-black text-slate-900 truncate" title={tx.merchant}>
                            {tx.merchant}
                          </span>
                          <span className={`text-xs font-black shrink-0 ${
                            tx.type === 'income' ? 'text-emerald-700' : 'text-slate-950'
                          }`}>
                        ...
                        <div className="flex items-center justify-between gap-2 mt-1">
                          ...
                          <select
                            ...
                            className="text-[11px] font-bold bg-white/90 hover:bg-white border border-slate-200 rounded-xl px-2.5 py-0.5 text-slate-800 focus:outline-none max-w-[140px] truncate cursor-pointer transition-colors shadow-sm"
                          >
                            {(categories || [])
        ...
        ══════════════════════════════════════════════════ */}
        {step === 'success' && (
          <div className="card p-8 flex flex-col items-center text-center gap-4 max-w-sm mx-auto my-auto">
            ...
            <div className="flex flex-col gap-1.5">
              <h3 className="text-base font-black text-slate-900">Выписка успешно импортирована!</h3>
            ...
            <button
              type="button"
              onClick={onBack}
              className="w-full py-3.5 rounded-full bg-slate-950 hover:bg-slate-900 text-white text-xs font-black transition-all shadow-md mt-2 active:scale-95"
            >
      ...
      {/* ── Fixed Floating Bottom Bar on Preview Step - Liquid Glass ── */}
      {step === 'preview' && (
        <div className="fixed bottom-0 left-0 right-0 z-40 dock-glass border-t border-white/90 p-4 safe-pb shadow-2xl flex flex-col gap-3 max-w-md md:max-w-lg mx-auto rounded-t-[28px]">
          {/* Member selector */}
          <div className="flex items-center justify-between px-1">
            <span className="text-xs font-bold text-slate-600">Записать расходы на:</span>
            <div className="flex items-center gap-1 p-0.5 rounded-2xl bg-white/60 border border-white/80 shadow-sm">
              {(members || []).map(m => {
          ...
          <div className="flex gap-2.5">
            <button
              type="button"
              onClick={resetAll}
              className="py-3.5 px-4 rounded-2xl bg-white/80 hover:bg-white border border-white/90 text-slate-800 text-xs font-bold transition-all shrink-0 shadow-sm"
            >
              Другой файл
            </button>
            <button
              ...
              className="flex-1 py-3.5 px-4 rounded-2xl bg-slate-950 hover:bg-slate-900 active:scale-[0.98] text-white text-xs font-black shadow-xl shadow-slate-950/20 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:pointer-events-none"
            >
              {loading ? (
                <>
